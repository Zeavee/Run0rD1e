...
public class ChatActivityTest {
    ...
    private void chatActivityTest2Helper1(int id1, int pos1, ViewAction... viewActions) {
        ViewInteraction appCompatEditText3 = onView(allOf(withId(id1), childAtPosition(childAtPosition(childAtPosition(allOf(withId(
                android.R.id.content), childAtPosition(withId(R.id.decor_content_parent), 0)), 0), 1), pos1), isDisplayed()));
    ...
    public void chatActivityTest2() throws InterruptedException {
        ...
        ViewInteraction constraintLayout = onView(allOf(childAtPosition(allOf(withId(R.id.recyclerQueryFriends),
                childAtPosition(childAtPosition(allOf(withId(android.R.id.content), childAtPosition(withId(R.id.decor_content_parent), 0)), 0), 0)), 0), isDisplayed()));
        ...
        chatActivityTest2Helper1(R.id.messageField, 0, replaceText("hi"), closeSoftKeyboard());

        chatActivityTest2Helper1(R.id.sendMessageButton, 1, click());

        /*ViewInteraction appCompatEditText3 = onView(allOf(withId(R.id.messageField), childAtPosition(childAtPosition(childAtPosition(allOf(withId(
                android.R.id.content), childAtPosition(withId(R.id.decor_content_parent), 0)), 0), 1), 0), isDisplayed()));
        appCompatEditText3.perform(replaceText("hi"), closeSoftKeyboard());

        ViewInteraction appCompatImageButton = onView(allOf(withId(R.id.sendMessageButton), childAtPosition(childAtPosition(childAtPosition(
                allOf(withId(android.R.id.content), childAtPosition(withId(R.id.decor_content_parent), 0)), 0), 1), 1), isDisplayed()));
        appCompatImageButton.perform(click());*/

        pressBack();

        ViewInteraction appCompatButton5 = onView(allOf(withId(R.id.chat_button), withText("chat"), childAtPosition(childAtPosition(
                allOf(withId(R.id.friends_recyclerview), childAtPosition(childAtPosition(withClassName(is("androidx.coordinatorlayout.widget.CoordinatorLayout")),
                        1), 0)), 0), 2), isDisplayed()));
        appCompatButton5.perform(click());

        ViewInteraction textView = onView(allOf(withId(R.id.message_body), withText("hi"), childAtPosition(childAtPosition(allOf(withId(R.id.messages_view),
                childAtPosition(childAtPosition(withId(android.R.id.content), 0), 0)), 1), 0), isDisplayed()));
